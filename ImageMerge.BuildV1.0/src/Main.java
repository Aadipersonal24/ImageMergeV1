...
public class Main {
	...
	public static void main(String[] args) {
		...
		//Image Merge By Orca Corp.
		//Build Version 0.1
		//Some Errors may occur as it is still in beta
		System.out.println();
		System.out.println("Enter path to Backgrounds folder : ");
		String path_BG = sc.nextLine();
		System.out.println("Enter path to foregrounds folder : ");
		String path_FG = sc.nextLine();
		System.out.println("Enter path to output folder : ");
		...
		for(int i = 0; i < array_BG.length; i++) {
			for(int j = 0; j < array_FG.length; j++) {
				int folder = i+1;
				String fileName = outPath + "\\" + folder + "\\" + array_BG[i].getName() + " _ " + array_FG[j].getName();
				try {
	...
	static void mergeImage(String img_path_bg, String img_path_fg, String name) {
		try {
			...
			BufferedImage outputIMG = new BufferedImage(width_BG, height_BG, BufferedImage.TYPE_INT_ARGB);
			Graphics2D render = outputIMG.createGraphics();
			render.drawImage(image_BG, 0, 0, null);
			render.drawImage(image_FG, 0, 0, null);
			
			ImageIO.write(outputIMG, "PNG", new File(name + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
